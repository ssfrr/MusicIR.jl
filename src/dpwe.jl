...
function pvsample{T<:Real}(b::Array{Complex{T}}, t::AbstractArray, hop::Integer=(size(b, 1)-1))
    ...
    # Empty output array
    c = Array(eltype(b), rows, length(t))

    # Expected phase advance in each bin
    ϕ₀ = zeros(div(N,2)+1)
    ϕ₀[2:end] = 2pi*hop/N .* [1:div(N, 2)]
    ...
    b = hcat(b, zeros(eltype(b), rows,1))
    ...
    #    dp = angle(bcols[:, 2]) - angle(bcols[:, 1]) - ϕ₀
    ...
    # TODO - cache magnitudes and phases to use in future iterations
    for tt in t
        icol = ifloor(tt)+1
        offset = tt - icol
        for row in 1:rows
            # average the magnitudes
            mag = (1-offset)*abs(b[row, icol]) + offset*(abs(b[row, icol+1]))
            c[row, ocol] = mag * cis(ph[row])
            # calculate the phase, assuming we're within ±π of the expected
            dϕ = angle(b[row, icol]) - angle(b[row, icol+1])
            dϕ = phasewrap(dϕ - ϕ₀[row]) + ϕ₀[row]
...
function istft{T<:Real}(d::Array{Complex{T}}, ftsize=2*(size(d,1)-1), h=div(ftsize, 4), win=hanning(ftsize))
    # istft(d, F, W, H)
    ...
    timeframes::Array{T, 2} = irfft(d, ftsize, 1)
